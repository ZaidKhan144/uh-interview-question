...
function accordionExpandCollapseButtons() {
  ...
  const toggleAccordionItems = (element, toggle) => {
    const accordion = $(element.parentElement.parentElement).children('.accordion')
    const accordionItems = $(accordion).children('.accordion-item')

    for (let i = 0; i < accordionItems.length; i++) {
      const collapseButton = $(accordionItems[i]).children('.accordion-button');
      const collapse = $(accordionItems[i]).children('.accordion-collapse');
      ...
      if (toggle) {
        if (collapseButton.hasClass('collapsed')) {
          if (!collapse.hasClass('collapsing')) {
            console.log(collapseButton, bsCollapse, collapse);
            bsCollapse.show();
          }
  ...
  //get all accordion-toggles on the page
  const accordionToggles = document.querySelectorAll('.accordion-toggles');
  if (!accordionToggles || accordionToggles.length < 1) return; 
  
  for (let i = 0; i < accordionToggles.length; i++) {
    const expandAll = $(accordionToggles).children('.expand-all');
    const collapseAll = $(accordionToggles).children('.collapse-all');

    if (expandAll.length === 1) expandAll.on('click', (e) => toggleAccordionItems(e.currentTarget, true))
    if (collapseAll.length === 1) collapseAll.on('click', (e) => toggleAccordionItems(e.currentTarget, false))
  }
}
